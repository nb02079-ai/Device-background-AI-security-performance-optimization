...
> **Test 미사용 원칙**: 본 Phase의 모든 평가는 **Validation**에서 수행되었다. `test_idx`는 저장만 하고 어떤 코드에서도 로드하지 않는다(프로토콜 §9).
...
# --- 3. 공식 분할 보존 + train을 Train/Validation으로 재분할 (§2.1, §11.2.1) ---
is_test = df['test'].astype(bool)
dev_idx = df.index[~is_test]
test_idx = df.index[is_test]

tr_idx, va_idx = train_test_split(
    dev_idx, train_size=0.75, random_state=SEED_SPLIT, stratify=y.loc[dev_idx]
)
...
```
특징 수: 14
Train 30000 / Validation 10000 / Test 24747
  Train 악성 50.0%   Val 악성 50.0%   Test 악성 78.8%
중복 쌍: [('n_events','n_uniq_child',0.9829), ('n_events','n_uniq_cguid',1.0)]
제거 특징: ['n_uniq_child', 'n_uniq_cguid']
최종 특징 수: 12
```
...
|---|---|---|---|---|---|---|---|
| A: RF 기본 | 0.8405±0.0007 | 0.9193 | 0.9288 | 0.9101 | 0.9725 | 0.0698 | [[4651,349],[450,4550]] |
| **B: RF+BO** | **0.8472±0.0010** | 0.9215 | 0.9420 | 0.9020 | 0.9771 | **0.0556** | [[4722,278],[490,4510]] |
| C: RF+BO+스태킹 | 0.8452±0.0007 | 0.9210 | 0.9369 | 0.9057 | **0.9777** | 0.0610 | [[4695,305],[472,4528]] |
| D: MLP | 0.6309±0.0034 | 0.7872 | 0.8877 | 0.7072 | 0.8903 | 0.0896 | [[4552,448],[1464,3536]] |

학습 시간(시드당): A 약 8초 / B 약 2.7초 / C 약 42초 / D 약 4초

### 4.2 차이의 95% 신뢰구간 (대응 비교, §8)

시드 5개를 짝지어 차이를 계산하고 t분포로 CI를 산출했다. **개별 CI의 중첩이 아니라 차이 자체의 CI**로 판정한다.

| 비교 | Δ MCC | 95% CI | 판정 |
|---|---|---|---|
| 튜닝 효과 (A→B) | +0.0067 | [+0.0046, +0.0088] | 차이 관측됨 |
| 스태킹 효과 (B→C) | **−0.0020** | [−0.0036, −0.0003] | **오히려 하락** |
| 튜닝+스태킹 (A→C) | +0.0047 | [+0.0038, +0.0056] | 차이 관측됨 |
| 구조 차이 (D→A) | **+0.2096** | [+0.2052, +0.2140] | 차이 관측됨 |

### 4.3 임계값 분석 (§8)

Youden's J 조정 시 부트스트랩 500회로 임계값 CI를 산출한다. CI 폭 > 0.2이면 불안정으로 보아 0.5로 회귀한다.

| 모델 | Youden J | 부트스트랩 95% CI | 폭 | MCC(0.5) | MCC(J) | 판정 |
|---|---|---|---|---|---|---|
| A | 0.590 | [0.522, 0.628] | 0.107 | 0.8394 | 0.8472 | 안정 |
| B | 0.515 | [0.486, 0.551] | 0.065 | 0.8490 | 0.8512 | 안정 |
| C | 0.601 | [0.549, 0.631] | 0.082 | 0.8449 | 0.8508 | 안정 |
| D | 0.482 | [0.457, 0.507] | 0.051 | 0.6301 | 0.6316 | 안정 |

네 모델 모두 안정 판정이나 MCC 개선폭은 0.002~0.008로 작다. **Test의 사전확률이 78.8%로 Validation(50%)과 다르므로(§11.2.2), 여기서 선택한 임계값이 Test에서 최적이라는 보장은 없다.**
...
B→C에서 MCC가 0.0020 하락했고 CI가 0을 포함하지 않는다. 다만 이는 앙상블 이론과 부합한다.
...
| `tr_idx.npy` / `va_idx.npy` / `test_idx.npy` | 분할 인덱스 (test는 미사용) |
